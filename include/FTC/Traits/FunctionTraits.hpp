...
template <typename F> struct arg_count_of;
template <typename F> inline constexpr std::size_t arg_count_of_v = arg_count_of<F>::value;
...
/// Get the argument count of a callable value
template <typename F> constexpr auto ArgCountOf(F &&f);
...
template <typename F, std::size_t N> struct arg_at
{
    static_assert(N < arg_count_of_v<F>, "arg_at: Argument index out of range");
